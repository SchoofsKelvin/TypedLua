...
local TostringValue, TupleToString, List, pprint = do
	...
	function TupleToString(...)
		local res = {...}
		for i = 1, select('#', ...) do
			res[i] = TostringValue(res[i])end
		return table concat(res, ",\32")end
	
	function List(tab)print()
		for k, v in pairs(tab) do
			print("-\32["..TostringValue(k, true).."]\32=", TostringValue(v, true)..';')end end
	...
	local function path()
		...
		trace = {}
		for i = 1, #res do
			trace[i] = res[#res-i+1]end
		return table concat(trace, '\62')end
	...
	BLOCKS = 0
	function pprint(...)
		print('['..level()..'|'..BLOCKS..']', TupleToString(...))end end
...
--[[ Parser stuff]]do
	...
	function Parser Find(pat, plain)
		return self str:find(pat, self i, plain)end
...
--[[ Syntax parsing]]do
	...
	function Parser Stat()
		...
		elseif
			...
			while self:Keyword("elseif")do
				...
				assert(expr, "Expected\32an expression")
				assert(self:Keyword("then"), "Expected\32`then`")
				blocks[#blocks+1] = {expr(self:Block())}end
			local otherwise = 
	...
	function Parser CheckBinop(expr)
		...
		return self:PostExpression(expr)end
	...
	function Parser StringConstant()
		...
		return{
			["Line"] = self:Line()
			["Type"] = "Constant"
			["Value"] = self str:sub(pos, a-1)}end
	...
	function Parser Funcbody()
		...
		return{
			...
			["Type"] = "Function"
			["Chunk"] = chunk
			["Parameters"] = params}end end




function Parser Splitprint()
	splitprint(self str, self i)end
...
xpcall(function()
	...
	f:write(TostringValue(res))f:close()end, 
function(e)
	print("ERROR:", e)
